...
//这个参数不统一，的确很傻，但是这样的参数方便很多，又好像用不到
static std::tuple<float, float, float> computeBarycentric3D(const Vector3f& p, const std::array<Eigen::Vector3f, 3>& view_pos)
{

	float triangleArea =   (view_pos[0]- view_pos[2]).cross(view_pos[1]- view_pos[2]).norm();
	float c1 = (p - view_pos[2]).cross(view_pos[1] - view_pos[2]).norm() / triangleArea;
	float c2 = (p - view_pos[2]).cross(view_pos[0] - view_pos[1]).norm() / triangleArea;
	std::cout << " this is c1,c2,c3 : " << c1 << "  " << c2 << "  " << 1 - c1 - c2 << std::endl;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
	...
	for (int x = floor(LeftBound)-1; x <= ceil(RightBound)+1; ++x)
		for (int y = floor(BottomBound)-1; y <= ceil(TopBound)+1; ++y)
		{
			auto[alpha, beta, gamma] = computeBarycentric2D(x,y,t.v);
			float w_reciprocal = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
			float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
			z_interpolated *= w_reciprocal;
			int tpos = get_index(x, y);
			if (insideTriangle(x, y, t.v) && z_interpolated < depth_buf[tpos])
			{
				...
				//set_pixel(Eigen::Vector2i(x, y), Eigen::Vector3f(255,0,0));
				depth_buf[tpos] = z_interpolated;
			}	
		}
